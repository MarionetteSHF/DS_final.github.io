...
df = read_csv("data/clean_data.csv") %>% 
  ...
  mutate(
   ...
   reason=factor(reason, levels =c("Heavy Traffic","Mechanical Problem","Flat Tire","Other","Won't Start","Problem Run","Accident","Dealyed by School","Late return from Field Trip","Weather Conditions")),
    run_type=factor(run_type,levels =c("Special Ed AM Run","General Ed AM Run",    
"Special Ed PM Run","General Ed PM Run","Special Ed Field Trip","General Ed Field Trip","Project Read PM Run"," Project Read AM Run" ))) %>% 
...
cv_df=df %>% 
  ...
  mutate(lm_1 = map(.x = train,~ lm((daily_delay) ~ month + day +reason+ prcp + run_type + snwd,family = poisson(),data = .x)),
    lm_2 = map(.x = train, ~ lm((daily_delay) ~month + day + reason + prcp + run_type + snwd + day*reason,family = poisson(),data = .x))) %>% 
  select(-train)
```
...
```{r,eval = FALSE}
cv_df %>% 
  mutate(rmse_1 = map2_dbl(lm_1, test, ~rmse(model = .x, data = .y)),
