...
Our interested dependent variable is number of delay across three school years: the amount of delay in Manhattan. Based on the graph below, the daily delay across three consecutive school year have declined.
...
## Cross Validation 
Predictors we chose above are statistically significant.However,we notice that interaction terms are not contributing to model predictability.We would say lm_3 will be better than other two models because its root mean square error seems to be smaller. Therefore, lm_3 could be useful for predicting the total number of bus delay in Manhattan.
```{r,message=FALSE}
cv_df %>% 
  mutate(
