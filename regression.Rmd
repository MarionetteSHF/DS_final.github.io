...
cv_df=df %>% 
  ...
  mutate(lm_1 = map(.x = train,~ lm((daily_delay) ~ month + day + prcp  + snwd,data = .x)),
    lm_2 = map(.x = train, ~ lm((daily_delay) ~month + prcp +  snwd ,data = .x))) 
```
...
multi.fit=lm(daily_delay~month + day + prcp  + snwd, data=df_1)
...
```{r,eval = FALSE}
#test$reasonlevel <- Map(union, test$reasonlevel, c("reason","run_type"))
cv_df %>% 
  mutate(
